...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get venue distribution by city (since venueType doesn't exist)
    const venuesByCity = await prisma.venue.groupBy({
      by: ['city'],
      _count: { id: true },
    });

    const totalVenueCount = venuesByCity.reduce((sum, item) => sum + item._count.id, 0);
    const formattedVenuesByCity = venuesByCity.map(item => ({
      city: item.city,
      count: item._count.id,
    ...
    const formattedSubscriptionStats = subscriptionStats.map(item => ({
      plan: item.planType,
      count: item._count.id,
      revenue: item.planType === 'professional' ? item._count.id * 29 : 0, // Mock revenue calculation
    }));
    ...
    return NextResponse.json({
      ...
      venuesByCity: formattedVenuesByCity,
